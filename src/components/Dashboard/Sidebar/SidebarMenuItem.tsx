    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `flex items-center gap-3 font-display rounded-[7px] transition-all duration-300 transform relative min-w-0 ${
          isCollapsed
            ? `justify-center px-3 ${isActive ? "py-2.5" : "py-1.5"}`
            : `px-4 ${isActive ? "py-2.5" : "py-1.5"}`
        } ${
          isActive
    ...
    >
      ...
      {!isCollapsed && (
        <Tooltip title={label} placement="right">
          <span className="text-[14px] md:text-[15px] lg:text-[16px] font-medium truncate whitespace-nowrap flex-1 min-w-0">
            {label}
          </span>
        </Tooltip>
      )}
